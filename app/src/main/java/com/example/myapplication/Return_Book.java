...
public class Return extends AppCompatActivity {
            ...
            public void onComplete(@NonNull Task<QuerySnapshot> task) {
                ...
                if (task.isSuccessful()) {
                    if (task.getResult().size() == 1) {
                        ...
                        res1 = false;
                        p.cancel();
                        Toast.makeText(Return.this, "No Such User !", Toast.LENGTH_SHORT).show();
                    }
                } else {
                    res1 = false;
                    p.cancel();
                    Toast.makeText(Return.this, "Try Again !", Toast.LENGTH_SHORT).show();
                }
            ...
            public void onComplete(@NonNull Task<DocumentSnapshot> task) {
                if (task.isSuccessful()) {
                    if (task.getResult().exists()) {
                        ...
                        res2 = false;
                        p.cancel();
                        Toast.makeText(Return.this, "No Such Book !", Toast.LENGTH_SHORT).show();
                    }
                } else {
                    res2 = false;
                    p.cancel();
    ...
    private void returnBook()
    {
        ...
        if (getUser()&getBook())
        {
            ...
            {
                p.cancel();
                Toast.makeText(Return.this, "Given Book is not issued to the User !", Toast.LENGTH_SHORT).show();
                return;
            }
                        ...
                        db.document("Book/" + B.getId()).set(B).addOnCompleteListener(new OnCompleteListener<Void>() {
                            ...
                            public void onComplete(@NonNull Task<Void> task) {
                                if (task.isSuccessful()) {
                                    ...
                                    p.cancel();
                                    Toast.makeText(Return.this, "Book Returned Successfully !", Toast.LENGTH_SHORT).show();
